...
export default {
  "loads all 82 domains": async () => {
    const domains = loader.getAllDomains();
    assert.equal(domains.length, 82, `Expected 82 domains, got ${domains.length}`);
  },
  ...
  "getIndex returns valid structure": async () => {
    ...
    assert.equal(idx.total_entries, 82);
  ...
  "searchByCategory finds foundational": async () => {
    ...
    assert.equal(results.length, 15, `Expected 15 foundational entries, got ${results.length}`);
  },
